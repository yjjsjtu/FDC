...
def find_compensation_size(motor_position_config):
    ...
    if size == 0:
        raise RuntimeError("Could not find motor_position.sources.0.compensation_table in config. Please verify connection and that the device supports compensation tables.")
...
async def run_inertial_compensation(args, m, s, ax):
    ...
    measured_velocity_values = numpy.array(measured_velocity_values)
    valid = ~numpy.isnan(measured_velocity_values)
    if not valid.any():
        raise RuntimeError("No data captured. The motor may not be rotating. Try increasing --voltage")
    elif not valid.all():
        x = numpy.arange(len(measured_velocity_values))
        measured_velocity_values = numpy.interp(
            x, x[valid], measured_velocity_values[valid],
            period=len(measured_velocity_values))
...
async def main():
    ...
    try:
        ...
        print("DEBUG: Calling s.flush_read()...")
        await s.flush_read()
        print("DEBUG: Calling m.set_stop()...")
        await m.set_stop()
        
        try:
            print(f"DEBUG: Restoring position_min to {position_min}...")
            await asyncio.wait_for(
                s.command(f'conf set servopos.position_min {position_min}'.encode('utf8')),
                timeout=2.0)
            
            print(f"DEBUG: Restoring position_max to {position_max}...")
            await asyncio.wait_for(
                s.command(f'conf set servopos.position_max {position_max}'.encode('utf8')),
                timeout=2.0)
        except asyncio.TimeoutError:
            print("WARNING: Timeout restoring position limits, skipping to save compensation table.")
            
        print("DEBUG: Cleanup block complete.")
