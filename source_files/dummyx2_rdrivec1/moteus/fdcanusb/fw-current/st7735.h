...
class ST7735 {
 ...
 private:
  ...
  void FillRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
    ...
    // Fill our line buffer with the color
    uint8_t hi = color >> 8;
  ...
  void DrawChar2x(uint16_t x, uint16_t y, char c,
    ...
    for (int row = 0; row < 16; row++) {
      ...
      for (int col = 0; col < 12; col++) {
        ...
        line_buf_[col * 2] = is_fg ? fg_hi : bg_hi;
        line_buf_[col * 2 + 1] = is_fg ? fg_lo : bg_lo;
      }
      SpiDmaSend(line_buf_, 12 * 2, true);
    }
    CsHigh();
  }
  ...
  void DrawCharBold(uint16_t x, uint16_t y, char c,
    ...
    for (int row = 0; row < out_h; row++) {
      ...
      SpiDmaSend(line_buf_, out_w * 2, true);
    }
    CsHigh();
  }
  ...
  uint8_t update_row_ = 0;  // cycles 0鈫1鈫2鈫0 for incremental updates
  
  uint8_t line_buf_[WIDTH * 2];
