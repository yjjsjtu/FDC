...
class CanManager::Impl {
 public:
  ...
  void Poll() {
    ...
    // Append INA226 power measurements: V<mV> A<mA>
    {
      char numbuf[12];
      write_str(" V", 2);
      int len = snprintf(numbuf, sizeof(numbuf), "%ld",
                         static_cast<long>(power_info_.voltage_mv));
      if (len > 0) { write_str(numbuf, len); }
      write_str(" A", 2);
      len = snprintf(numbuf, sizeof(numbuf), "%ld",
                     static_cast<long>(power_info_.current_ma));
      if (len > 0) { write_str(numbuf, len); }
    }
  ...
  CanManager::PowerInfo power_info_;
...
void CanManager::SetPowerInfo(const PowerInfo& info) {
  impl_->power_info_ = info;
}
