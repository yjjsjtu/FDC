...
class ST7735 {
 public:
  ...
  void DisplayPower(int32_t voltage_mv, int32_t current_ma) {
    ...
    FormatValue(val, sizeof(val), milli_val);

    // Redraw only the number area for this row
    // First clear the text area, then draw the new value
    FillRect(TEXT_X, ROW_Y[update_row_] + 1,
             UNIT_X - TEXT_X - 1, CHAR_H, 0x0000);
    DrawStringBold(TEXT_X, ROW_Y[update_row_] + 1,
                   val, 0xFFFF, 0x0000);
 ...
 private:
  ...
  void InitGpio() {
    ...
    gpio.Pin = opts_.sck_pin;  HAL_GPIO_Init(opts_.sck_port, &gpio);
    gpio.Pin = opts_.mosi_pin; HAL_GPIO_Init(opts_.mosi_port, &gpio);
  ...
  __attribute__((always_inline)) inline void SpiByte(uint8_t byte) {
    uint32_t set_mosi = opts_.mosi_pin;
    uint32_t clr_mosi = (uint32_t)opts_.mosi_pin << 16;
    uint32_t set_sck  = opts_.sck_pin;
    uint32_t clr_sck  = (uint32_t)opts_.sck_pin << 16;
    volatile uint32_t* mosi_bsrr = &opts_.mosi_port->BSRR;
    volatile uint32_t* sck_bsrr  = &opts_.sck_port->BSRR;

#define SPI_BIT(n) \
    *mosi_bsrr = (byte & (1 << n)) ? set_mosi : clr_mosi; \
    *sck_bsrr = set_sck; \
    __NOP(); __NOP(); \
    *sck_bsrr = clr_sck; \
    __NOP(); __NOP()

    SPI_BIT(7); SPI_BIT(6); SPI_BIT(5); SPI_BIT(4);
    SPI_BIT(3); SPI_BIT(2); SPI_BIT(1); SPI_BIT(0);
#undef SPI_BIT
  }
  ...
  void SckHigh()  { opts_.sck_port->BSRR = opts_.sck_pin; }
  void SckLow()   { opts_.sck_port->BSRR = (uint32_t)opts_.sck_pin << 16; }
  void MosiHigh() { opts_.mosi_port->BSRR = opts_.mosi_pin; }
  void MosiLow()  { opts_.mosi_port->BSRR = (uint32_t)opts_.mosi_pin << 16; }
  ...
  uint8_t update_row_ = 0;  // cycles 0鈫�1鈫�2鈫�0 for incremental updates
};
