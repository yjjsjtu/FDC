...
int main(void) {
  ...
  // ST7735 160x80 LCD on software SPI
  fw::ST7735 lcd([&]() {
  ...
  // LCD backlight on PB8 - set high to enable
  DigitalOut lcd_backlight{PB_8, 1};
  ...
  persistent_config.Register("lcd", &lcd_config, [&](){
    lcd_backlight.write(lcd_config.enabled ? 1 : 0);
  });
  ...
  while (true) {
    ...
    if (new_time != old_time_ms) {
      ...
      if (tenms_count >= 10) {
        ...
        // Poll INA226 every 100ms (10 * 10ms), if LCD is enabled
        hundredms_count++;
        if (hundredms_count >= 10) {
          hundredms_count = 0;
          if (lcd_config.enabled) {
            ina226.Poll();
            ...
            // Update LCD: one row per call (~2ms), cycles U鈫扞鈫扨
            lcd.DisplayPower(ina226.voltage_mv(), ina226.current_ma());
          }
        }
